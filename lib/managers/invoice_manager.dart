...
class InvoiceManager {
  /// Step 1: Generate unsigned UBL invoice XML
  ...
  /// Step 2: Save XML to disk
  Future<String> saveInvoiceXml(
    XmlDocument xmlDoc,
  ...
  /// Step 3: Sign the XML with OpenSSL
  Future<String> signInvoice(String xmlPath) async {
  ...
  /// Step 4: Inject signature (XAdES) into XML
  Future<XmlDocument> injectSignatureIntoInvoice({
    ...
    // Read certificate
    final certificateString = await File(certificatePath).readAsBytes();
    final certificateBase64 = base64.encode(certificateString);
    // final certificateBase64 = base64Encode(certificateString);
    // Build SignedProperties (replace with your real XAdES SignedProperties)
    final signedProperties = XmlDocument.parse(
      '<SignedProperties Id="xadesSignedProperties"/>',
    );

    // Canonicalize invoice and generate hashes
    final normalizedInvoice = canonicalizeXml(invoice.toXmlString());
    final invoiceHash = generateInvoiceHash(normalizedInvoice);
    final signedPropertiesHash = generateInvoiceHash(
      canonicalizeXml(signedProperties.toXmlString()),
    );

    // Build SignedInfo
    final signedInfo = buildSignedInfo(
      invoiceHashBase64: invoiceHash,
      signedPropertiesHashBase64: signedPropertiesHash,
    );
    ...
    // Inject signature into invoice (UBLExtensions)
    final signedInvoice = injectSignature(
      invoice: invoice,
  ...
  /// Step 5: Complete flow: generate, sign, inject, save
  Future<String> generateAndSignInvoice({
    ...
    final unsignedInvoice = await generateUnsignedInvoice(
      invoiceNumber: invoiceNumber,
      items: items,
    ...
    //  Save unsigned invoice temporarily
    final xmlPath = await saveInvoiceXml(unsignedInvoice, invoiceNumber);

    //  Sign XML
    final signaturePath = await signInvoice(xmlPath);

    //  Inject signature
    final signedInvoiceDoc = await injectSignatureIntoInvoice(
      invoice: unsignedInvoice,
      signaturePath: signaturePath,
      certificatePath: Constants.certPath,
    );

    //  Save final signed invoice
    final finalPath = await saveInvoiceXml(signedInvoiceDoc, invoiceNumber);
    return finalPath;
  }

  ///  Prepare DTO for submission
  Future<Map<String, String>> prepareInvoiceSubmission({
    ...
    final canonicalXmlString = canonicalizeXml(xmlString).trim();

    //  Encode canonical XML to UTF-8 bytes
    final canonicalBytes = utf8.encode(canonicalXmlString);

    //  Compute SHA256 hash of canonical bytes
    final hashBytes = sha256.convert(canonicalBytes).bytes;

    //  Base64 encode canonical XML and hash (remove any newlines)
    final xmlBase64 = base64.encode(canonicalBytes).replaceAll('\n', '');
    final hashBase64 = base64.encode(hashBytes).replaceAll('\n', '');

    //  Generate UUID for submission
    final uuid = const Uuid().v4();

    return {"uuid": uuid, "invoice_hash": hashBase64, "invoice": xmlBase64};
